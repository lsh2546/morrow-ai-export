...
export async function GET() {
  await ensureSchema();
  const db = getD1();
  const [users, analyses, gemini, approvals, feedback, payments, logs] = await db.batch([
    ...
    db.prepare("SELECT COUNT(*) AS value FROM agent_payments WHERE status = 'executed'"),
  ...
  return NextResponse.json({
    ...
    approvals: value(approvals), feedback: value(feedback), payments: value(payments), logs: logs.results ?? [],
  });
}
